...
public:
    ...
    T&   Pop() ;            // returns and removes item from front of list
    T&   Pull() ;           // returns and removes item from end of list
    T&   Peek(int index);        
    // if index if non-negative Peek(i) return the ith item from the front of the
    ...
    // Peek(-1) returns the end item, Peek(-2) returns the item second from the end, and so on. 
    // Any Peek() access outside the range of the list throws and exception
    string toString();
private:
